...
TAG="run080_restart_per_repeat_textclip_matrix"
REPEATS=2
TIMEOUT_SEC=75
HOST="127.0.0.1:7861"
...
usage() {
  cat <<'EOF'
Usage:
  tools/run_q6p_restart_per_repeat_textclip_matrix.sh [options]

Options:
  --model <file>         Model key (default: trace021 key).
  --tag <value>          Output matrix tag (default: run080_restart_per_repeat_textclip_matrix).
...
Notes:
  ...
  - Forces modifier/autoencoder to none in this matrix to isolate text/clip pin effects.
EOF
}
...
echo "== restart-per-repeat text/clip matrix =="
echo "tag=$TAG"
