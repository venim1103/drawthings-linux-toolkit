...
set_entry_fields() {
  ...
  python3 - "$CUSTOM_JSON" "$entry_name" "$file_key" "$clip_key" "$default_scale" <<'PY'
import json
import pathlib
import sys
...
run_case() {
  ...
  echo "== case: $case_id =="
  echo "file=$file_key clip_encoder=$clip_key default_scale=$default_scale"

  set_entry_fields "$ENTRY_NAME" "$file_key" "$clip_key" "$default_scale"
  ...
  echo -e "${case_id}\t${file_key}\t${clip_key}\t${default_scale}\t${rc}\t${result}\t${signature}\t${canary_rc}\t${post_echo_rc}\t${responses}\t${images}\t${audio}\t${case_tag}" >> "$RESULTS_TSV"
}
...
echo -e "case\tentry_file\tentry_clip_encoder\tentry_default_scale\trc\tresult\tsignature\tcanary_rc\tpost_echo_rc\tresponses\timages\taudio\tcanary_tag" > "$RESULTS_TSV"
...

{
  ...
  echo "## Results"
  echo
  echo "| case | entry_file | entry_clip_encoder | default_scale | rc | result | signature | canary_rc | post_echo_rc | responses | images | audio |"
  echo "|---|---|---|---:|---:|---|---|---:|---:|---:|---:|---:|"
