...
TAG="run081_restart_per_repeat_textgate_clip_matrix"
REPEATS=2
TIMEOUT_SEC=75
HOST="127.0.0.1:7861"
...
CASES=(
  ...
  "text_clipvit_only|clip_vit_l14_f16.ckpt|none"
  "text_clipvit_clip_traced|clip_vit_l14_f16.ckpt|10_e_v1_bf16_regen_0_q6p.ckpt"
  "text_none_clip_traced|none|10_e_v1_bf16_regen_0_q6p.ckpt"
)

usage() {
  cat <<'EOF'
Usage:
  tools/run_q6p_restart_per_repeat_textgate_clip_matrix.sh [options]

Options:
  --model <file>         Model key (default: trace021 key).
  --tag <value>          Output matrix tag (default: run081_restart_per_repeat_textgate_clip_matrix).
  --repeats <n>          Repeats per case (default: 2).
...
Notes:
  ...
  - Forces modifier/autoencoder to none in this matrix to isolate text/clip companion effects.
  - Focuses on gemma-vs-clip text path gating with clip companion toggles.
EOF
}
...
echo "== restart-per-repeat text-gate clip matrix =="
echo "tag=$TAG"
