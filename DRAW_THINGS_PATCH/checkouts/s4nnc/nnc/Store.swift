...
extension DynamicGraph {
  ...
  public func openStore<SuccessResult>(
    ...
    if flags.contains(.readOnly) {
      ...
      sqlite3_exec(_sqlite, "PRAGMA auto_vacuum=incremental", nil, nil, nil)
      // drawthings-linux-toolkit: conversion/quantization writes are one-shot and
      // regenerable; the per-commit fsync + synchronous journal flush dominate
      // wall-clock on the Linux/WSL virtual disk (a 44GB convert spent ~46 min in
      // I/O wait). Disabling synchronous keeps journaling (VACUUM/rollback still
      // work) while letting writes stream through the page cache. Big speedup.
      sqlite3_exec(_sqlite, "PRAGMA synchronous=OFF", nil, nil, nil)
    }
    let store = Store(
