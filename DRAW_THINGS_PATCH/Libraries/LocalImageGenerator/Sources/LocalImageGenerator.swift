...
extension LocalImageGenerator {
  ...
  private func generateTextOnly(
    ...
    if modelVersion == .ltx2_3 {
      Self.ltx23Trace(
        "LocalImageGenerator.textEncoderFiles model=\(file) files=\(textEncoderFiles)"
      )
    }
    ...
    return graph.withNoGrad {
      ...
      let textEncodings = modelPreloader.consumeTextModels(
        {
          if modelVersion == .ltx2_3 {
            Self.ltx23Trace(
              "LocalImageGenerator.beforeTextEncode tokenLengthUncond=\(tokenLengthUncond) tokenLengthCond=\(tokenLengthCond)"
            )
          }
          let encoded = textEncoder.encode(
            tokenLengthUncond: &tokenLengthUncond, tokenLengthCond: &tokenLengthCond,
          ...
          if modelVersion == .ltx2_3 {
            Self.ltx23Trace(
              "LocalImageGenerator.afterTextEncode condCount=\(encoded.0.count) condShape0=\(encoded.0.first?.shape ?? []) textModelCount=\(encoded.1.count)"
            )
          }
          return encoded
        }(),
      ...
      if modelVersion == .ltx2_3 {
        Self.ltx23Trace("LocalImageGenerator.beforeFeedback signpost=textEncoded")
      }
