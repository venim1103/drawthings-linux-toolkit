...
private func LTX2SelfAttention(
  ...
  let mapper: ModelWeightMapper = { _ in
    ...
    // QK-norms multiply the de-interleaved q/k -> de-interleave them too
    // (drawthings-linux-toolkit; same head config as to_q/to_k).
    mapping["\(prefix).k_norm.weight"] = ModelWeightElement(
      [normK.weight.name], interleaved: true, numberOfHeads: h, headDimension: k)
    mapping["\(prefix).q_norm.weight"] = ModelWeightElement(
      [normQ.weight.name], interleaved: true, numberOfHeads: h, headDimension: k)
    return mapping
  }
  return (mapper, Model([x, rot], [out]))
}
...
private func LTX2CrossAttention(
  ...
  let mapper: ModelWeightMapper = { _ in
    ...
    if let normK = normK {
      mapping["\(prefix).k_norm.weight"] = ModelWeightElement(
        [normK.weight.name], interleaved: true, numberOfHeads: h, headDimension: k.1)
    }
    mapping["\(prefix).q_norm.weight"] = ModelWeightElement(
      [normQ.weight.name], interleaved: true, numberOfHeads: h, headDimension: k.1)
    return mapping
  }
  return (
...
private func LTX2CrossAttentionFixed(
  ...
  let mapper: ModelWeightMapper = { _ in
    ...
    mapping["\(prefix).to_v.weight"] = [toValues.weight.name]
    mapping["\(prefix).to_v.bias"] = [toValues.bias.name]
    // QK-norm multiplies the de-interleaved k -> de-interleave it too
    // (drawthings-linux-toolkit; same head config as to_k).
    mapping["\(prefix).k_norm.weight"] = ModelWeightElement(
      [normK.weight.name], interleaved: true, numberOfHeads: h, headDimension: k.1)
    return mapping
  }
