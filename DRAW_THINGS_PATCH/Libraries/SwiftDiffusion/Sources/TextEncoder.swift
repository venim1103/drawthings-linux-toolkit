...
extension TextEncoder {
  private func ltx23Trace(_ message: String) {
    ...
    -> ([DynamicGraph.Tensor<FloatType>], [Model])
  {
    ...
    if !weightsCache.detach(filePaths[0], to: textModel.parameters) {
      ltx23Trace(
        "encodeLTX2.loading_text_model path=\(URL(fileURLWithPath: filePaths[0]).lastPathComponent) externalOnDemand=\(externalOnDemand)"
      )
      // If we have more than 24GiB RAM, and not forced to be on demand. We load the whole thing (better for weights cache).
      // Move Qwen 2.5 VL to on-demand.
      TensorData.makeExternalData(for: filePaths[0], graph: graph)
    ...
    ltx23Trace(
      "encodeLTX2.text_model_ready outputLayers=\(outputHiddenStates.count) hiddenStatesShape=\(hiddenStates.shape)"
    )
    let lora = Array(
      (OrderedDictionary<String, LoRAConfiguration>(
        lora.filter({ $0.version == version }).map {
    ...
    ltx23Trace(
      "encodeLTX2.loading_text_feature_extractor path=\(URL(fileURLWithPath: filePaths[1]).lastPathComponent) shouldRunLoRASeparately=\(shouldRunLoRASeparately) loraCount=\(lora.count)"
    )
    graph.openStore(
      filePaths[1], flags: .readOnly,
      externalStore: TensorData.externalStore(filePath: filePaths[1])
    ...
    ltx23Trace("encodeLTX2.loaded_text_feature_extractor")
    ...
    ltx23Trace(
      "encodeLTX2.end conditioningCount=\(c.count) conditioningShape0=\(c.first?.shape ?? []) tokenLengthUncond=\(tokenLengthUncond) tokenLengthCond=\(tokenLengthCond)"
    )
    return (c, [textModel])
  }
