...
public class GRPCServerAdvertiser {
  ...
  public func startAdvertising(port: Int32, TLS: Bool) {
    let serviceName = TLS ? "\(name)_tls" : "\(name)_notls"
    #if canImport(C_DNSAdvertiser)
      if let IP = IP(), let localhost = localhost() {
        let hostname = localhost + ".local"
        nameHostnameIPAndPort = (serviceName, hostname, IP, UInt16(port))
        queue.async { [weak self] in
          guard let self = self else { return }
          self.phase = .probe(0)
          self.advertise(name: serviceName, hostname: hostname, IP: IP, port: UInt16(port))
        }
      }
    #elseif canImport(ObjectiveC)
      ...
      let netService = NetService(
        domain: serviceDomain, type: serviceType, name: serviceName, port: port)
